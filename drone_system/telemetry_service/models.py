            ...
            for j in range(i+1, len(drones)):
                ...
                for step_index , (apos, bpos) in enumerate(zip(drones[i].intermediate_steps, drones[j].intermediate_steps)):
                    ...
                    if congestionchecking_dist <= CONGESTION_DISTANCE:
                        actual_hypotenues = math.sqrt((drones[i].x-apos[0])**2 + (drones[i].y-apos[1])**2) # this is the st. line dist from x pos of ith drone to the would have been place of ith drone where congestion is happening
                        future_path_of_drone2_from_congestion = drones[j].intermediate_steps[step_index:]
                        congestion_dict.append({"id1":drones[i].id, "id2":drones[j].id, "time":round(time, 2), "step":step_index, "path_of_drone_2":future_path_of_drone2_from_congestion})
